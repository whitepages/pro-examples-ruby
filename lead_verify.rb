...
uri = URI('https://proapi.whitepages.com/3.1/lead_verify.json')
uri.query = URI.encode_www_form(params)

response_hash = JSON.parse(open(uri).read)

available_checks = %w(name_checks phone_checks address_checks email_address_checks ip_address_checks)
